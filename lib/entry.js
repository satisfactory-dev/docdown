...
function getValue(entry, tagName) {
  ...
  if (tagName == 'alias') {
    ...
    var error = (_.get(tag, 'errors') || [])[0] || undefined;
    if (error) {
...
class Entry {
  ...
  getCall() {
    if (this.#getCall == undefined) {
      var result = (_.get(/\*\/\s*(?:function\s+)?([^\s(]+)\s*\(/.exec(this.entry), 1) || '').trim();
      if (!result) {
        result = (_.get(/\*\/\s*(.*?)[:=,]/.exec(this.entry), 1) || '').trim();
        result = /['"]$/.test(result)
          ? result.replace(/^['"]*([^'"].*[^'"])['"]*$/, '$1')
      ...
      params.forEach(function(param) {
        ...
        var parentIndex = parentParam == null ? -1 : params.findIndex(function(param) {
   ...
   *
   * @todo replace _.get with foo?.bar syntax when we can drop node < 14
   */
  getReturns() {
    if (this.#getReturns == undefined) {
      ...
      var
        ...
        type = `${_.get(tag, 'type.name') || ''}` || unionReturn || '*';
